...
Sios API nodes. When images are requested from the API node,
...
from sios.api.common import size_checked_iter
from sios.api.v1 import images
from sios.common import exception
from sios.common import utils
from sios.common import wsgi
import sios.db
from sios import image_cache
...
class CacheFilter(wsgi.Middleware):
    ...
    def _process_v2_request(self, request, image_id, image_iterator):
        ...
        db_api = sios.db.get_api()
        image_repo = sios.db.ImageRepo(request.context, db_api)
        image = image_repo.get(image_id)
        image_meta = sios.notifier.format_image_notification(image)
        self._verify_metadata(image_meta)
        response = webob.Response(request=request)
