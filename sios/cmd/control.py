...
RELOAD_SERVERS = ['sios-api', 'sios-registry']
GRACEFUL_SHUTDOWN_SERVERS = ['sios-api', 'sios-registry',
                             'sios-scrubber']
...
def pid_files(server, pid_file):
    ...
    if pid_file:
        ...
        if os.path.exists('/var/run/sios/%s.pid' % server):
            pid_files = ['/var/run/sios/%s.pid' % server]
...
def add_command_parsers(subparsers):
    ...
    parser.set_defaults(servers=['sios-' + s for s in ALL_SERVERS])
